...
export default async function (
  ...
  try {
    publishedVersion = (await runCommandLine(`npm view ${name.join(`/`)} version`)).trim()
  } catch (e) {
  ...
  const pathToPackageJson = path.join.apply(path, name.concat([`package.json`]))
  ...
  console.log(`Latest published version "${publishedVersion}" does not match latest built version "${packageJson.version}"; publishing...`)
  await runCommandLine(`npm publish ${path.join.apply(path, name.slice())}`)
}
