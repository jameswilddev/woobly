...
describe(`@woobly/build-tool-helpers`, () => {
  describe(`determinePurposeOfFilePath`, () => {
    describe(`when plugins are not installed`, () => {
      ...
      it(
        `throws an error for paths which are not valid TypeScript or content`,
        () => expect(() => determinePurposeOfFilePath({}, path.join(`src`, `subdirectory-a`, `subdirectory-b`, `not-a-type-script-file-and-not-a-valid-content-path!.test-unimplemented-file-extension`)))
          .toThrowError(`Unable to determine the purpose of file "${path.join(`src`, `subdirectory-a`, `subdirectory-b`, `not-a-type-script-file-and-not-a-valid-content-path!.test-unimplemented-file-extension`)}".  If this is intended to be content, please ensure that it is of the form described in the documentation.`),
      )

      it(
        `throws an error for paths which are valid content for which no plugin is installed`,
